export default function StatusBadge({ status }) {
  const styles = {
    Открыта: 'bg-blue-50 text-blue-700 border-blue-200',
    'В работе': 'bg-amber-50 text-amber-700 border-amber-200',
    Ожидание: 'bg-purple-50 text-purple-700 border-purple-200',
    Закрыта: 'bg-emerald-50 text-emerald-700 border-emerald-200',
    Архив: 'bg-slate-100 text-slate-600 border-slate-200',
  };

  const currentStyle =
    styles[status] || 'bg-slate-50 text-slate-700 border-slate-200';

  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${currentStyle}`}
    >
      {status}
    </span>
  );
}
